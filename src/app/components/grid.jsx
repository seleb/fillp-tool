...
})
{
	...
	const [colorBg, colorFill] = useMemo(() => [coloursHex[colours[0]], coloursHex[colours[1]]], [colours]);
	useLayoutEffect(() =>
	{
		refDraw.current = new Draw1Bit({ width: width * tileX, height: height * tileY, colorBg, colorFill });
	}, []);
	useLayoutEffect(() =>
	{
	...
	useEffect(() =>
	{
		...
		{
			onChange(2 ** ((width * height - 1) - ((event.detail.x % width) + (event.detail.y % height) * width)));
		}
	...
	useLayoutEffect(() =>
	{
		Object.values(bits).forEach((on, idx, a) =>
		{
			...
			for (let x = 0; x < tileX; ++x)
			{
				...
				{
					refDraw.current.fill(idx % width + x * width, Math.floor(idx / width) + y * height, on);
				}
			}
		})
		refDraw.current.render();
	...
	return (
		<div ref={refContainer} class="grid" />
	);
}
